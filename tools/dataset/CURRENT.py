...
while again == 1:
    ...
    for file in datasetarray:
        ...
        with h5py.File(file, 'r') as f:
            ...
            for dset, key in traverse_datasets(f):
                if f[dset].shape == (240, 320, 3):
                    ...
                    if SAVE:
                        ...
                        file_name = file[:-5] + "/" + file_name + ".png"
                        im.save(file_name)
                        i = i + 1
                        foundImg = 1
                if 'steering' in dset:
            ...
            values = np.array(h)
            ii = \
            np.where(np.logical_and(values >= (max(h) - (0.015 * max(h))), values <= (max(h) + (0.015 * max(h)))))[
                0]
            ...
            if defaults == 'y':
                ...
                if window == 'd':
                    window = 5
                floor = input("The current floor is " + str(
                    100 * math.floor(h[0] / 100)) + ". press d to continue with this, otherwise input floor\n")
                if floor == 'd':
                    floor = 100 * math.floor(h[0] / 100)
                ceil = input("The current ceiling is " + str(
                    100 * math.ceil(h[0] / 100)) + ". press d to continue with this, otherwise input ceiling\n")
                if ceil == 'd':
                    ceil = 100 * math.ceil(h[0] / 100)
                ...
                if tooclose == 'd':
                    tooclose = 10
                amtofimages = input(
                    "How many images in the negative folders? (default = 200, all, or input your own")
                if amtofimages == 'all':
                    numofimg = 'n'
            ...
            for vi in iii:
                falsepos = False
                if math.isclose(percentage_diff(h[vi], h[min(len(h) - 1, vi + 1)]), percentage_diff(h[vi], h[min(len(h) - 1, vi + 2)]), abs_tol=1):
                    if percentage_diff(h[vi], h[min(len(h) - 1, vi + 10)]) - percentage_diff(h[vi], h[min(len(h) - 1, vi + 1)]) > 1.5:
                        if percentage_diff(h[vi], h[min(len(h) - 1, vi + 15)]) > 10:
                            falsepos = False
                        elif percentage_diff(h[vi], h[min(len(h) - 1, vi + 20)]) > 20:
                            falsepos = False
                        else:
                            falsepos = True
                    else:
            ...
            while (indexofroc != len(roc)):
                ...
                while rocarrayframes[-1] >= len(h):
                    rocarrayframes.pop(-1)
                ...
                if changeldmrk > 0 and change == False:
                    iii[countldmrks] = maxindex - changeldmrk
                    print(
                        "Warning: Landmark at frame " + str(iii[countldmrks]) + " is inaccurate, increase window!")
            ...
            for ldmarks in iii:
                ...
                if ldmarks - zzz < tooclose:
                    ...
                    for val in [ldmarks, zzz]:
                        if val in iii:
                            iii.pop(iii.index(val))
            ...
            if folderorfile == 'file' and sys.platform != 'linux':
                figure.show()
            ...
            while (change != 'no' and folderorfile == "file"):
                ...
                if change == 'no':
                    ...
                    for yy in iii:
                        zoomed = px.line(x=range(yy - int(c), yy + int(c) + 1),
                                         y=[h[find] for find in range(yy - int(c), yy + int(c) + 1)])
                        zoomed.add_vline(x=yy, line_color='red')
                        zoomed.update_xaxes(nticks=2 * int(c))
                        if sys.platform != 'linux':
                            zoomed.show()
                        action = input(
                            "What action would you like to take about this landmark? (n = N/A, d = del, c = change, exit)\n")
                        if action == 'n':
                            print("No change")
                        elif action == 'd':
                    ...
                    zoomed = px.line(x=range(addldmrk - int(c), min(len(h) - 1, addldmrk + int(c) + 1)),
                                     y=[h[find] for find in range(addldmrk - int(c), min(len(h) - 1, addldmrk + int(c) + 1))])
                    zoomed.add_vline(x=addldmrk, line_color='red')
                    zoomed.update_xaxes(nticks=2 * int(c))
                    if sys.platform != 'linux':
                        zoomed.show()
                    addldmrk = input(
                        "What landmark would you like to add? (input frame of landmark again to confirm)\n")
                    iii.append(int(addldmrk))
                    iii.sort()
                    print("Added landmark at " + str(addldmrk) + "\n")
                elif change == 'new':
                    iii = [int(i) for i in input().split()]
                    break
            ...
            iv[:] = [iv.zfill(5) + ".png" for iv in iv]
