...
from models.Variational import load_model
...
from utils.utils import str2bool, get_imgId2landmarkId, per_landmark
...
parser.add_argument('-epoch_pre', '--epoch_pre', default=15, type=int, required=False,
                    help='Number of epochs in pre-training (default: 15)')
parser.add_argument('-epoch_fine', '--epoch_fine', default=40, type=int, required=False,
                    help='Number of epochs in fine-tuning (default: 40)')
parser.add_argument('-batch_pre', '--batch_pre', default=36, type=int, required=False,
                    help='Batch size in pre-training (default: 36)')
...
parser.add_argument('-sum_dist', '--sum_dist', default=False, type=str2bool, required=False,
                    help='Whether or not to sum elementwise distances (default: False)')
...
dir_coco = '12_3_3'
dir_output = 'output'
dir_ckpt = '/path/to/save/models'
course_name = 'dual_fisheye_exclude_six_{}'.format(args.name)
...
print(savename)
if sum_dist:
    print('Summing elementwise distances')
                    ...
                    for i, img in enumerate(dataloader_test):
                        ...
                        if qry_imgs is None:
                            ...
                            match, probabilities = MatchDetector(
                                model, qry_imgs, lm_proto, lm_eigs, probabilities, threshold=0.5, sum_dist=sum_dist, device=device)
...
model = load_model(FCdim=FC_len, backbone=backbone, device=device, skip_cov=skip_cov, sum_dist=sum_dist)
...
if not os.path.exists(osp.join(dir_output, 'figures')):
    os.makedirs(osp.join(dir_output, 'figures'))
...
n_episodes = 100
lr = 1e-5
savename = course_name +'_FineTune_batch' + str(batch_fine) +'_' + str(sup_size) + '-shot' + '_lr_' + str(lr) + '_lrsch_' + str(sch_param_2) + '_' + str(sch_param_1) + '_' + str(n_episodes) + 'episodes'
print(savename)
