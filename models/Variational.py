...
def load_model(**kwargs):
    ...
    sum_dist = kwargs['sum_dist']

    print('Using backbone: {}'.format(backbone))
    if backbone == 'resnet50_swav':
        encoder = torch.hub.load('facebookresearch/swav:main', 'resnet50')  # SSL pretrained
    elif backbone == 'resnet50':
        encoder = models.resnet50(pretrained=True)  # supervised pretrained
    ...
    return Variational(encoder, cov, classifier, device, sum_dist)


class Variational(nn.Module):
    def __init__(self, encoder, cov_module, classifier, device, sum_dist=None):
