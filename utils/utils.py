...
def exponential_moving_average(values, window_size, smoothing=2):
    ema = [values[0]]
    for i in range(1, len(values)):
        weight = smoothing / (1 + i) if i < window_size else smoothing / (1 + window_size)
        ema.append((1 - weight) * ema[i-1] + weight * values[i])
    return ema
